...
#weights_fname = "third.model" if args.arch == "tf" else "mnet2.model"
...
while True:
    ...
    #cv2_img = cv2.morphologyEx(blur, cv2.MORPH_CLOSE, kernel)
    #cv2_img = cv2.GaussianBlur(cv2_img, (31,31), 0)
    #    seq.append(cv2_img)

    #cv2_img = cv2.addWeighted(seq[0], 0.2, seq[1], 0.3, 10)
    #cv2_img = cv2.addWeighted(seq[2], 0.5, cv2_img, 0.5, 10)
